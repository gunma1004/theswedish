...
// 📍 서울·경기·인천 + 충청권(천안·아산·대전·청주) 전지역 데이터 통합
...
> = {
  seoul: {
    name: "서울특별시",
    districts: {
      jongno: { name: "종로구", dongs: ["청운동", "효자동", "사직동", "삼청동", "부암동", "평창동", "무악동", "교남동", "가회동", "종로1.2.3.4가동", "종로5.6가동", "이화동", "혜화동", "창신동", "숭인동"] },
      ...
      gangseo: { name: "강서구", dongs: ["등촌동", "화곡동", "우장산동", "가양동", "발산동", "공항동", "방화동", "염창동", "마곡동"] },
      ...
      gwanak: { name: "관악구", dongs: ["보라매동", "청림동", "성현동", "행운동", "낙성대동", "청룡동", "은천동", "신원동", "서림동", "신사동", "신림동", "봉천동"] },
  ...
  gyeonggi: {
    name: "경기도",
    districts: {
      suwon: { name: "수원시", dongs: ["인계동", "매탄동", "영통동", "광교동", "권선동", "세류동", "호매실동", "화서동", "정자동", "조원동", "파장동"] },
      seongnam: { name: "성남시 (분당·판교)", dongs: ["정자동", "서현동", "야탑동", "이매동", "수내동", "판교동", "삼평동", "백현동", "운중동", "신흥동", "태평동", "상대원동"] },
      goyang: { name: "고양시 (일산·덕양)", dongs: ["백석동", "마두동", "장항동", "정발산동", "식사동", "탄현동", "주엽동", "대화동", "행신동", "화정동", "삼송동"] },
      yongin: { name: "용인시 (수지·기흥)", dongs: ["풍덕천동", "죽전동", "상현동", "성복동", "신갈동", "기흥동", "보정동", "동백동", "마북동", "역북동"] },
      bucheon: { name: "부천시", dongs: ["중동", "상동", "심곡동", "원미동", "소사동", "역곡동", "송내동", "오정동", "원종동"] },
      ansan: { name: "안산시", dongs: ["고잔동", "중앙동", "초지동", "선부동", "본오동", "사동", "이동", "일동", "와동"] },
      anyang: { name: "안양시 (평촌)", dongs: ["평촌동", "범계동", "비산동", "호계동", "관양동", "안양동", "석수동", "박달동"] },
      hwaseong: { name: "화성시 (동탄)", dongs: ["동탄동", "병점동", "진안동", "반월동", "봉담읍", "향남읍", "남양읍", "새솔동"] },
      pyeongtaek: { name: "평택시 (고덕·송탄)", dongs: ["고덕동", "비전동", "동삭동", "세교동", "서정동", "송탄동", "안중읍", "포승읍"] },
      siheung: { name: "시흥시 (배곧)", dongs: ["배곧동", "정왕동", "은계동", "대야동", "신천동", "은행동", "목감동", "장현동"] },
      gimpo: { name: "김포시 (한강신도시)", dongs: ["구래동", "장기동", "운양동", "마산동", "풍무동", "사우동", "고촌읍", "통진읍"] },
      paju: { name: "파주시 (운정)", dongs: ["운정동", "야당동", "동패동", "금촌동", "문산읍", "조리읍"] },
      uijeongbu: { name: "의정부시", dongs: ["의정부동", "호원동", "장암동", "신곡동", "민락동", "금오동", "가능동"] },
      namyangju: { name: "남양주시 (다산·별내)", dongs: ["다산동", "별내동", "평내동", "호평동", "와부읍", "진접읍", "화도읍"] },
      hanam: { name: "하남시 (미사·위례)", dongs: ["미사동", "망월동", "풍산동", "덕풍동", "신장동", "위례동"] },
      gwangmyeong: { name: "광명시 (철산·일직)", dongs: ["철산동", "하안동", "소하동", "일직동", "광명동"] },
      gunpo: { name: "군포시 (산본)", dongs: ["산본동", "당동", "당정동", "부곡동", "금정동"] },
      osan: { name: "오산시", dongs: ["원동", "궐동", "청학동", "오산동", "세교동"] },
      icheon: { name: "이천시", dongs: ["창전동", "중리동", "증포동", "부발읍", "장호원읍"] },
      anseong: { name: "안성시", dongs: ["공도읍", "대덕면", "안성동", "당왕동", "아양동"] }
    }
  },
  incheon: {
    name: "인천광역시",
    districts: {
      ...
      seogu: { name: "서구 (청라·검단)", dongs: ["청라동", "검단동", "원당동", "당하동", "마전동", "가정동", "석남동", "연희동"] },
      michuhol: { name: "미추홀구 (주안)", dongs: ["주안동", "용현동", "학익동", "도화동", "숭의동", "관교동", "문학동"] },
      junggu: { name: "중구 (영종)", dongs: ["운서동", "중산동", "운남동", "영종동", "신포동", "동인천동"] },
      gyeyang: { name: "계양구", dongs: ["계산동", "작전동", "효성동", "서운동", "임학동"] }
    }
  },
  cheonan: {
    name: "천안시",
    districts: {
      seobuk: { name: "천안시 서북구 (불당·두정)", dongs: ["불당동", "두정동", "성정동", "백석동", "쌍용동", "차암동", "성성동", "직산읍", "성거읍", "입장면"] },
      dongnam: { name: "천안시 동남구 (신부·청수)", dongs: ["신부동", "청수동", "청당동", "원성동", "봉명동", "신방동", "목천읍", "풍세면", "병천면"] }
    }
  },
  asan: {
    name: "아산시",
  ...
  daejeon: {
    name: "대전광역시",
    districts: {
      yuseong: { name: "유성구 (봉명·도안)", dongs: ["봉명동", "상대동", "도안동", "장대동", "궁동", "노은동", "지족동", "반석동", "관평동", "전민동"] },
      seo: { name: "서구 (둔산·월평·탄방)", dongs: ["둔산동", "탄방동", "월평동", "만년동", "갈마동", "괴정동", "용문동", "가장동", "도마동", "관저동"] },
      jung: { name: "중구 (은행·선화·대흥)", dongs: ["은행동", "선화동", "대흥동", "문창동", "대사동", "오류동", "태평동", "유천동", "문화동"] },
      dong: { name: "동구 (용전·가양)", dongs: ["용전동", "가양동", "자양동", "가오동", "판암동", "홍도동", "삼성동", "성남동"] },
      daedeok: { name: "대덕구 (송촌·신탄진)", dongs: ["송촌동", "중리동", "법동", "비래동", "오정동", "신탄진동", "석봉동"] }
    }
  },
  cheongju: {
    name: "청주시",
    districts: {
      heungdeok: { name: "청주시 흥덕구 (복대·가경)", dongs: ["복대동", "가경동", "봉명동", "송절동", "강서동", "비하동", "오송읍", "옥산면"] },
      seowon: { name: "청주시 서원구 (산남·분평)", dongs: ["산남동", "분평동", "수곡동", "성화동", "개신동", "모충동", "사직동", "사창동"] },
      cheongwon: { name: "청주시 청원구 (율량·오창)", dongs: ["율량동", "주성동", "우암동", "내덕동", "오창읍", "내수읍"] },
      sangdang: { name: "청주시 상당구 (용암·금천)", dongs: ["용암동", "금천동", "탑동", "대성동", "영운동", "방서동", "동남지구"] }
    }
  }
};
